...
Point multiply(Point P, BigInteger n){
 ...
 void sharedSecret(){  // Figure 10.7 of book
    ...
    void encryptionForB(){  // page 305 of book
        ...
        Point kG = this.multiply(G, k); // k times G
        //kG = this.multiply(G, k);
        //kG.x = G.x.multiply(k);
        //kG.y = G.y.multiply(k);
        Point kY = null; // k times publicKeyB
        kY = this.multiply(publicKeyB, k);
        //kY.x = publicKeyB.x.multiply(k);
        //kY.y = publicKeyB.y.multiply(k);
        BigInteger mu;  // message times kY.x mod p
        mu = message.multiply(kY.x);
        mu = mu.mod(p);
        System.out.println("encrypted message:/n");
        System.out.println(kG); System.out.println(mu.toString(16));
        // (kG, mu) is the encrypted message
        Point kY2 = null;  // B computes kY as privateKeyB times kG
        kY2 = this.multiply(kG, privateKeyB);
        kY2.x = privateKeyB.multiply(kG.x);
        kY2.y = privateKeyB.multiply(kG.y);
        BigInteger decodedMessage; // kY2.x modinverse times mu mod p
        decodedMessage = kY2.x.modInverse(p);
        decodedMessage = decodedMessage.multiply(mu);
        decodedMessage = decodedMessage.mod(p);
        System.out.println("decrypted message:/n");
